...
test("공통 잠금 리소스 타입만 허용한다", () => {
  for (const value of ["project", "task", "personal_note", "shipment", "employee", "comment", "setting", "material_usage_request", "material_usage_group", "glass_cost_statement", "coating_cost_statement", "accessory_item", "project_accessory_usage"]) assert.equal(isEditingLockResourceType(value), true);
