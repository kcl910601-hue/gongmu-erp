...
export type CurrentEmployee = {
  ...
  organization?: { id?: number; name: string | null } | Array<{ id?: number; name: string | null }> | null;
};
...
export async function getEmployeeByAuth(
  ...
  const byAuthUser = await client
    .from("employees")
    .select("id, name, email, position, role, active, approval_status, auth_user_id, organization:organizations(id, name)")
