...
export async function queryRawMaterialContracts(supabase: SupabaseClient, params: URLSearchParams) {
  ...
  if (error) return { data: null, error }; const summaries = await queryContractAllocationSummaries(supabase, data ?? []); if (summaries.error || !summaries.data) return { data: null, error: summaries.error }; const records = (data ?? []).map((record) => { const supplierRow = Array.isArray(record.supplier) ? record.supplier[0] : record.supplier; return { ...record, supplier_name: supplierRow?.name ?? null, supplier: undefined, allocation_summary: summaries.data.get(record.id) } as RawMaterialContract; }); return { data: records, error: null };
}
