alter table public.material_usage_requests
  drop constraint material_usage_requests_target_check;

alter table public.material_usage_requests
  add constraint material_usage_requests_target_check check (
    (allocation_type = 'project' and project_id is not null and destination_name is null)
    or (
      allocation_type = 'factory'
      and project_id is null
      and (destination_name is null or destination_name in ('공장재고-중문', '공장재고-자동문'))
    )
    or (
      allocation_type = any (array['as'::text, 'sample'::text, 'etc'::text])
      and project_id is null
      and nullif(btrim(destination_name), '') is not null
    )
  );

create or replace function public.create_unallocated_factory_material_usage_request_v2(
...
begin
  ...
  if p_destination_name not in ('공장재고-중문', '공장재고-자동문') then
    raise exception '공장재고 구분을 확인해주세요.' using errcode = '22023';
  end if;
  ...
  insert into public.material_usage_requests(
    ...
    'AL', 'factory', null, p_destination_name, p_quantity_tons,
...
revoke all on function public.create_unallocated_factory_material_usage_request_v2(text,text,numeric,date,text,text) from public;
grant execute on function public.create_unallocated_factory_material_usage_request_v2(text,text,numeric,date,text,text) to authenticated;
grant execute on function public.create_unallocated_factory_material_usage_request_v2(text,text,numeric,date,text,text) to service_role;
