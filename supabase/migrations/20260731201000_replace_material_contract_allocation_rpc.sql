...
drop function if exists public.save_material_contract_allocation(uuid, uuid, bigint, numeric, date, text, text, text, boolean);

create function public.save_material_contract_allocation(
  p_contract_id uuid,
...
begin
  ...
  elsif p_project_id is not null or nullif(btrim(p_destination_name), '') is null then
  ...
  if p_allocation_id is null then
    insert into public.material_contract_allocations (
      ...
      case when p_allocation_type = 'project' then null else nullif(btrim(p_destination_name), '') end,
    ...
    update public.material_contract_allocations set
      ...
      destination_name = case when p_allocation_type = 'project' then null else nullif(btrim(p_destination_name), '') end,
