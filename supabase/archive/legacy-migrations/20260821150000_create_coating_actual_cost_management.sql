begin;
alter type public.partner_type add value if not exists 'coating';
commit;
begin;
...
create table public.coating_cost_allocations(
 id uuid primary key default gen_random_uuid(), statement_id uuid not null references public.coating_cost_statements(id) on delete restrict,
...
create table public.coating_cost_allocation_history(
 id bigint generated always as identity primary key, allocation_id uuid not null references public.coating_cost_allocations(id) on delete restrict,
 statement_id uuid not null references public.coating_cost_statements(id) on delete restrict, project_id bigint not null references public.projects(id) on delete restrict,
...
create index coating_cost_statements_month_idx on public.coating_cost_statements(accounting_month);
create index coating_cost_statements_vendor_month_idx on public.coating_cost_statements(vendor_organization_id,accounting_month);
create index coating_cost_statements_status_month_idx on public.coating_cost_statements(status,accounting_month);
create index coating_cost_allocations_statement_idx on public.coating_cost_allocations(statement_id);
create index coating_cost_allocations_project_status_idx on public.coating_cost_allocations(project_id,status);
create index coating_cost_allocations_project_created_idx on public.coating_cost_allocations(project_id,created_at desc);
create index coating_cost_history_allocation_changed_idx on public.coating_cost_allocation_history(allocation_id,changed_at desc);

create function public.assert_coating_vendor() returns trigger language plpgsql set search_path=public,pg_temp as $$ begin
...
create function public.log_coating_cost_activity(p_type text,p_statement uuid,p_project bigint,p_title text,p_metadata jsonb) returns void language plpgsql security definer set search_path=public,pg_temp as $$
...
revoke all on function public.log_coating_cost_activity(text,uuid,bigint,text,jsonb) from public,anon,authenticated;

create function public.save_coating_cost_statement(p_id uuid,p_vendor bigint,p_month date,p_invoice text,p_supply bigint,p_vat bigint,p_memo text)
returns public.coating_cost_statements language plpgsql security definer set search_path=public,pg_temp as $$
...
create function public.void_coating_cost_statement(p_id uuid) returns public.coating_cost_statements language plpgsql security definer set search_path=public,pg_temp as $$ declare result public.coating_cost_statements%rowtype; begin
 if not public.is_approved_admin() then raise exception '관리자 권한이 필요합니다.' using errcode='42501'; end if;
...
create function public.save_coating_cost_allocation(p_statement uuid,p_project bigint,p_amount bigint,p_memo text,p_action text default 'save') returns public.coating_cost_allocations language plpgsql security definer set search_path=public,pg_temp as $$
...
revoke all on function public.save_coating_cost_statement(uuid,bigint,date,text,bigint,bigint,text),public.void_coating_cost_statement(uuid),public.save_coating_cost_allocation(uuid,bigint,bigint,text,text) from public,anon;
grant execute on function public.save_coating_cost_statement(uuid,bigint,date,text,bigint,bigint,text),public.void_coating_cost_statement(uuid),public.save_coating_cost_allocation(uuid,bigint,bigint,text,text) to authenticated;
alter table public.coating_cost_statements enable row level security; alter table public.coating_cost_allocations enable row level security; alter table public.coating_cost_allocation_history enable row level security;
...
create or replace function public.update_project_coating_cost_entry(p_statement_id uuid,p_project_id bigint,p_vendor_organization_id bigint,p_accounting_month date,p_supply_amount_krw bigint,p_vat_amount_krw bigint,p_invoice_number text default null,p_memo text default null)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function public.void_project_coating_cost_entry(p_statement_id uuid,p_project_id bigint)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare s public.coating_cost_statements%rowtype; a public.coating_cost_allocations%rowtype; active_count integer; active_amount bigint; active_project bigint;
begin
 if not public.is_approved_admin() then raise exception '관리자 권한이 필요합니다.' using errcode='42501'; end if;
...
revoke all on function public.create_project_coating_cost_entry(bigint,bigint,date,bigint,bigint,text,text),public.update_project_coating_cost_entry(uuid,bigint,bigint,date,bigint,bigint,text,text),public.void_project_coating_cost_entry(uuid,bigint) from public,anon;
grant execute on function public.create_project_coating_cost_entry(bigint,bigint,date,bigint,bigint,text,text),public.update_project_coating_cost_entry(uuid,bigint,bigint,date,bigint,bigint,text,text),public.void_project_coating_cost_entry(uuid,bigint) to authenticated;
