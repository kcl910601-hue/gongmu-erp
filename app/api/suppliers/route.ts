...
export async function POST(request: Request) {
  ...
  if (!name) return Response.json({ error: "AL업체명을 입력해주세요." }, { status: 400 });
  ...
  if (error) return Response.json({ error: error.code === "23505" ? "이미 등록된 AL업체입니다." : error.message }, { status: error.code === "23505" ? 409 : 500 });
