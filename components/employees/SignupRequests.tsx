...
type SignupRequest = {
  ...
  approval_status: "pending" | "rejected" | "incomplete";
...
export default function SignupRequests() {
  ...
  async function updateRequest(
    ...
    if (saving || request.approval_status === "incomplete") return;
    setSaving(true);
    setErrorMessage("");
    try {
            ...
            <tbody>
              {requests.map((request) => (
                <tr key={request.id} className="border-t border-slate-200">
                  ...
                  <td className="px-3 py-2">{request.created_at?.slice(0, 10) ?? "-"}</td>
                  ...
                  <td className="px-3 py-2 text-right">
                    {request.approval_status !== "incomplete" && (
                      <>
                        <button
                          type="button"
