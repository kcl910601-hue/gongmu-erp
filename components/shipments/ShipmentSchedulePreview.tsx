import {
  ...
  buildShipmentCalendarWeeksForRange,
  formatShipmentScheduleRange,
  formatShipmentQuantity,
  MAX_CALENDAR_ITEMS,
  WEEKDAY_LABELS,
  type ShipmentScheduleItem,
  type ShipmentScheduleOptions,
  type ShipmentScheduleRange,
...
export function ShipmentSchedulePreview({ month, range, vendorName, printedAt, items = [], options }: Props) {
  ...
  const weeks = range ? buildShipmentCalendarWeeksForRange(range, sortedItems) : buildShipmentCalendarWeeks(month, sortedItems);
  ...
  return (
    <div className="mx-auto min-w-[900px] bg-white p-8 text-[11px] text-slate-900 shadow-sm">
      <header className="mb-3 flex items-end justify-between border-b-2 border-blue-900 pb-2">
        <h2 className="text-2xl font-bold">조립업체 {range ? "기간" : "월간"} 출고 일정표</h2>
        <dl className="grid grid-cols-[auto_auto] gap-x-4 gap-y-1 text-right">
          <dt className="font-bold">업체명</dt><dd>{vendorName}</dd>
          <dt className="font-bold">{range ? "출력기간" : "출력월"}</dt><dd>{range ? formatShipmentScheduleRange(range) : month}</dd>
      ...
      {options.showCalendar && (
        <section>
          ...
          {(Array.isArray(weeks) ? weeks : []).map((week, weekIndex) => (
            <div key={weekIndex} className="grid break-inside-avoid grid-cols-7 [page-break-inside:avoid]">
