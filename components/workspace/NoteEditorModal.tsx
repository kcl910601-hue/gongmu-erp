...
import { dispatchPersonalNotesChanged, getNoteEditorDefaults, NOTE_EDITOR_OPEN_EVENT, type NoteEditorPreset, type PersonalNoteColor } from "@/lib/personal-notes";
...
export default function NoteEditorModal() {
  ...
  useEffect(() => {
    function open(event: Event) {
      const detail = (event as CustomEvent<{ preset?: NoteEditorPreset; dueDate?: string | null }>).detail;
      const preset = detail?.preset;
      setNoteType(preset === "todo" || preset === "sticky" ? preset : "memo");
      setTitle(""); setContent(""); setColor("default"); setDateMode(detail?.dueDate ? "custom" : "none"); setCustomDate(detail?.dueDate ?? ""); setError(""); setIsOpen(true);
    }
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/personal-notes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...getNoteEditorDefaults(noteType), title, content, color, dueDate }) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error ?? "저장하지 못했습니다.");
      dispatchPersonalNotesChanged(); setIsOpen(false); toast.success("내 업무가 추가되었습니다.");
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/40 p-4" onMouseDown={close}>
    <div role="dialog" aria-modal="true" aria-labelledby="note-editor-title" className="w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-5 shadow-xl" onMouseDown={(event) => event.stopPropagation()}>
      <div className="flex items-start justify-between"><div><p className="text-xs font-semibold text-blue-600">My Workspace</p><h2 id="note-editor-title" className="mt-1 text-xl font-bold text-slate-950">새 업무 추가</h2><p className="mt-1 text-sm text-slate-500">무엇을 추가하시겠습니까?</p></div><button type="button" aria-label="닫기" onClick={close} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100"><X size={18}/></button></div>
