...
import { getPersonalNoteAccess, type PersonalNote } from "@/lib/personal-notes";
...
export function PersonalNoteActions({ note, commentsOpen, timelineOpen, onEdit, onShare, onTogglePin, onDelete, onToggleComments, onToggleTimeline }: {
  ...
  return <div className="flex shrink-0 items-center gap-0.5" onClick={(event) => event.stopPropagation()}>
    ...
    {access.canComment && <button type="button" aria-label="댓글" onClick={onToggleComments} className={`${buttonClass} ${commentsOpen ? "text-blue-600" : ""}`}><MessageCircle size={14}/>{(note.comment_count ?? 0) > 0 && <span className="ml-0.5 text-[10px] font-bold">{note.comment_count}</span>}</button>}
