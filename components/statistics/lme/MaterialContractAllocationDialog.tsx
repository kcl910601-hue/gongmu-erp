...
import { MaterialUsageGroupSelector } from "@/components/statistics/lme/MaterialUsageGroupSelector";
...
import { buildMaterialUsageAllocationPreview, type MaterialUsageAllocationStrategy } from "@/lib/material-usage-requests";
...
type FormState = { allocationType: MaterialAllocationType; projectId: string; materialUsageGroupId: string | null; destinationName: string; quantityTons: string; allocationDate: string; status: "planned" | "confirmed"; purchaseOrderNo: string; memo: string };
const emptyForm = (fixedProject?: ProjectOption): FormState => ({ allocationType: "project", projectId: fixedProject ? String(fixedProject.id) : "", materialUsageGroupId: null, destinationName: "", quantityTons: "", allocationDate: new Date().toISOString().slice(0, 10), status: "planned", purchaseOrderNo: "", memo: "" });
const statusLabel = { planned: "예정", confirmed: "확정", cancelled: "취소" } as const;
type ExcessPlan = { requestedTons: number; availableTons: number; excessTons: number; contracts: Array<{ id: string; contractName: string; priceKrwPerKg: number; effectiveStartDate: string; availableTons: number }> };
...
export function MaterialContractAllocationDialog({ contract, fixedProject, initialAllocationId, startInCreateMode = false, onClose, onChanged }: { contract: RawMaterialContract | null; fixedProject?: ProjectOption; initialAllocationId?: string | null; startInCreateMode?: boolean; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  const [excessPlan, setExcessPlan] = useState<ExcessPlan | null>(null);
  const [strategy, setStrategy] = useState<MaterialUsageAllocationStrategy>("leave_unallocated");
  ...
  function startEdit(allocation: MaterialContractAllocation) {
    ...
    setForm({ allocationType: allocation.allocation_type, projectId: allocation.project_id === null ? "" : String(allocation.project_id), materialUsageGroupId: null, destinationName: allocation.destination_name ?? "", quantityTons: String(allocation.quantity_tons), allocationDate: allocation.allocation_date, status: allocation.status === "confirmed" ? "confirmed" : "planned", purchaseOrderNo: allocation.purchase_order_no ?? "", memo: allocation.memo ?? "" });
  }
  async function refresh() { await Promise.all([load(), onChanged()]); }
  async function save(selectedStrategy?: MaterialUsageAllocationStrategy) {
  ...
  return <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/45 p-4" role="dialog" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) onClose(); }}>
    <div className="max-h-[94vh] w-full max-w-[96vw] overflow-y-auto rounded-2xl bg-white p-5 xl:max-w-7xl">
      ...
      {editing&&<section className="mt-4 rounded-2xl border border-blue-200 bg-blue-50/30 p-4">{form.allocationType==="project"&&editing==="new"&&<label className="mb-3 block text-xs font-semibold">사용 구분<MaterialUsageGroupSelector projectId={form.projectId?Number(form.projectId):null} value={form.materialUsageGroupId} onChange={materialUsageGroupId=>setForm({...form,materialUsageGroupId})}/></label>}<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {excessPlan&&<div className="fixed inset-0 z-[130] flex items-center justify-center bg-slate-950/50 p-4"><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5"><h3 className="text-lg font-bold">계약 가용량을 초과합니다</h3><div className="mt-3 grid grid-cols-3 gap-2 text-xs"><div className="rounded-xl bg-slate-50 p-3">요청량<b className="mt-1 block">{formatNumber(excessPlan.requestedTons,4)}t</b></div><div className="rounded-xl bg-slate-50 p-3">현재 계약 가용<b className="mt-1 block">{formatNumber(excessPlan.availableTons,4)}t</b></div><div className="rounded-xl bg-red-50 p-3 text-red-700">초과량<b className="mt-1 block">{formatNumber(excessPlan.excessTons,4)}t</b></div></div><div className="mt-4 space-y-2 text-sm">{([['auto_split','다음 계약으로 자동 배정'],['increase_contract','현재 계약 증액 후 배정'],['leave_unallocated','잔여량까지만 배정하고 초과분 미배정']] as const).map(([value,label])=><label key={value} className="flex items-start gap-2 rounded-xl border p-3"><input type="radio" checked={strategy===value} onChange={()=>setStrategy(value)}/><span><b>{label}</b>{value==='auto_split'&&excessPlan.contracts.length===0&&<small className="block text-red-600">사용 가능한 다음 계약이 없습니다.</small>}</span></label>)}</div>{strategy==='increase_contract'&&<label className="mt-3 block text-xs font-semibold">증액 사유<textarea value={increaseReason} onChange={(event)=>setIncreaseReason(event.target.value)} rows={2} className="mt-1 w-full rounded-xl border p-2 text-sm"/></label>}{strategy==='auto_split'&&(()=>{const preview=buildMaterialUsageAllocationPreview(excessPlan.requestedTons,excessPlan.contracts.map(item=>({id:item.id,availableTons:item.availableTons,priceKrwPerKg:item.priceKrwPerKg,effectiveStartDate:item.effectiveStartDate})));return <div className="mt-4 rounded-xl bg-slate-50 p-3 text-xs"><b>배정 Preview</b>{preview.allocations.map((row)=><p key={row.contractId} className="mt-1">{excessPlan.contracts.find(item=>item.id===row.contractId)?.contractName} · {formatNumber(row.quantityTons,4)}t · {formatNumber(row.priceKrwPerKg)}원/kg</p>)}<p className="mt-2 font-semibold">총 배정 {formatNumber(preview.allocatedTons,4)}t · 미배정 {formatNumber(preview.unallocatedTons,4)}t · 예상 원가 {formatNumber(preview.estimatedCostKrw)}원</p></div>})()}<div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={()=>setExcessPlan(null)}>취소</Button><Button variant="primary" disabled={saving||strategy==='increase_contract'&&!increaseReason.trim()} onClick={()=>{setExcessPlan(null);void save(strategy);}}>선택한 방식으로 저장</Button></div></div></div>}
