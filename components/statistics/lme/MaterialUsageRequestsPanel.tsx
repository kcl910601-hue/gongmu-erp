...
function AllocationButton({row,onChanged}:{row:MaterialUsageRequest;onChanged:()=>Promise<void>;legacyOpen:(row:MaterialUsageRequest)=>Promise<void>}){
  ...
  async function save(){const kg=Number(form.quantityKg);const price=Number(form.price);const contract=options.find(item=>item.id===form.contractId);if(!Number.isFinite(kg)||kg<=0)return toast.error("배정량을 확인해주세요.");if(row.allocation_type!=="project"&&kg>row.unallocated_tons*1_000+0.05)return toast.error(`배정량은 최대 ${formatNumber(row.unallocated_tons*1_000,1)} kg입니다.`);if(form.source==="contract"&&!contract)return toast.error("계약을 선택해주세요.");if(form.source==="contract"&&kg>(contract?.allocation_summary.availableTons??0)*1_000+0.05)return toast.error(`계약 가용량은 ${formatNumber((contract?.allocation_summary.availableTons??0)*1_000,1)} kg입니다.`);if(form.source==="direct_price"&&(!Number.isFinite(price)||price<=0))return toast.error("직접단가를 확인해주세요.");setSaving(true);try{const response=await fetch("/api/statistics/lme/usage-requests",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({usageRequestId:row.id,sourceType:form.source,contractId:form.contractId,quantityTons:kg/1_000,directUnitPriceKrwPerKg:price,status:form.status,expectedAvailableTons:contract?.allocation_summary.availableTons})});const result=(await response.json())as{error?:string};if(!response.ok)throw new Error(result.error);scheduleCollaborationEvents([MATERIAL_USAGE_REQUESTS_CHANGED_EVENT]);setOpen(false);await onChanged();toast.success("배정했습니다.");}catch(error){toast.error(error instanceof Error?error.message:"배정하지 못했습니다.");}finally{setSaving(false);}}
  return<><IconButton label="배정 관리" onClick={()=>void show()}><PlusCircle size={14}/></IconButton>{open&&<Modal title="원자재 배정" onClose={()=>setOpen(false)}><div className="mb-3 grid grid-cols-3 gap-2"><Summary label="발주" value={row.quantity_tons}/><Summary label="기배정" value={row.allocated_tons}/><Summary label="미배정" value={row.unallocated_tons} warning/></div><div className="grid grid-cols-2 gap-2"><Button variant={form.source==="contract"?"primary":"outline"} onClick={()=>setForm({...form,source:"contract"})}>계약 배정</Button><Button variant={form.source==="direct_price"?"primary":"outline"} onClick={()=>setForm({...form,source:"direct_price"})}>직접 단가</Button></div>{form.source==="contract"&&<select value={form.contractId} onChange={event=>setForm({...form,contractId:event.target.value})} className="mt-3 w-full rounded-xl border px-3 py-2 text-sm"><option value="">계약 선택</option>{options.map(item=><option key={item.id} value={item.id}>{item.contract_name} · 가용 {formatNumber(item.allocation_summary.availableTons*1_000,1)} kg</option>)}</select>}<div className="mt-3 grid grid-cols-2 gap-3"><Field label="배정량(kg)" type="number" value={form.quantityKg} onChange={quantityKg=>setForm({...form,quantityKg})}/>{form.source==="direct_price"&&<Field label="직접단가(원/kg)" type="number" value={form.price} onChange={price=>setForm({...form,price})}/>}<label className="text-xs font-semibold">상태<select value={form.status} onChange={event=>setForm({...form,status:event.target.value})} className="mt-1 w-full rounded-xl border px-3 py-2 text-sm"><option value="planned">예정</option><option value="confirmed">확정</option></select></label></div>{form.source==="direct_price"&&Number(form.quantityKg)>0&&Number(form.price)>0&&<p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm font-semibold">배정원가 {formatNumber(Math.round(Number(form.quantityKg)*Number(form.price)))}원</p>}<div className="mt-5"><Actions saving={saving} confirm="배정" onCancel={()=>setOpen(false)} onConfirm={()=>void save()}/></div></Modal>}</>;
}
