...
export type ProjectCoatingCostRow = {
  id: string;
  statement_id: string;
...
export function ProjectCoatingCostSection({
  project,
  rows,
  total,
  canManage,
  ...
  const [vendors, setVendors] = useState<Vendor[]>([]),
    [editing, setEditing] = useState<ProjectCoatingCostRow | null>(null),
  ...
  useEffect(() => {
    if (!editable) return;
    void fetch("/api/partner-organizations?partner_type=coating", {
      cache: "no-store",
    }).then(async (response) => {
  ...
  function start(row?: ProjectCoatingCostRow) {
    setEditing(row ?? null);
    setForm(
      row
        ? {
  ...
  async function save() {
    ...
    try {
      const response = await fetch(
          `/api/statistics/project-margin/${project.id}/coating-costs`,
          {
      ...
      toast.success(
        editing ? "도장 원가가 수정되었습니다." : "도장 원가가 등록되었습니다.",
      );
      setOpen(false);
      setEditing(null);
      await onChanged();
  ...
  async function voidEntry(row: ProjectCoatingCostRow) {
    if (!confirm("이 도장 원가를 무효 처리하시겠습니까?")) return;
    const response = await fetch(
        `/api/statistics/project-margin/${project.id}/coating-costs`,
        {
          method: "PATCH",
    ...
    toast.success("도장 원가를 무효 처리했습니다.");
    await onChanged();
  }
  return (
    <section className="mt-4 rounded-xl border p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h3 className="font-semibold">도장 실제원가 상세</h3>
                ...
                <td>
                  ...
                  <td>
                    ...
                    <>
                      ...
                      href={`/statistics/coating-costs?statementId=${row.statement_id}`}
                    >
                        전체 정산 보기
                      </Link>
