...
export default function PersonalWorkspace() {
  ...
  return <section className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-bold text-slate-950">My Workspace</h2><p className="mt-0.5 text-sm text-slate-500">오늘 처리할 Todo를 중심으로 관리합니다.</p></div><button type="button" onClick={() => openNoteEditor("todo")} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700"><Plus size={16}/>Todo 추가</button></div>
    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{([{ label: "미완료", value: counts.open, filter: "open" }, { label: "오늘", value: counts.today, filter: "today" }, { label: "지연", value: counts.overdue, filter: "overdue" }, { label: "완료", value: counts.completed, filter: "completed" }] as const).map((item) => <button type="button" key={item.label} onClick={() => applySummary(item.filter)} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 text-left transition hover:border-blue-200 hover:bg-blue-50"><span className="text-xs font-medium text-slate-500">{item.label}</span><span className="ml-2 text-lg font-bold text-slate-900">{item.value}</span></button>)}</div>
    <div className="mt-4 flex flex-wrap gap-1 rounded-xl bg-slate-100 p-1">{tabOptions.map((tab) => <button type="button" key={tab.value} onClick={() => setPreferences((current) => ({ ...current, tab: tab.value }))} className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${preferences.tab === tab.value ? "bg-white text-blue-700 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>{tab.label}</button>)}</div>
    {error && <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">{error}</p>}

    {(preferences.tab === "todo" || preferences.tab === "all") && <div className="mt-4 grid items-start gap-4 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
      <div className="min-w-0 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
        ...
        <div className="mt-3 grid gap-2 lg:grid-cols-[minmax(220px,1fr)_auto_auto]"><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2"><Search size={15} className="text-slate-400"/><input value={search} onChange={(event) => { setSearch(event.target.value); setVisibleTodoCount(TODO_PAGE_SIZE); }} placeholder="Todo 제목과 내용 검색" className="min-w-0 flex-1 bg-transparent text-sm outline-none"/></label><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-2"><ListFilter size={14} className="text-slate-400"/><select value={preferences.filter} onChange={(event) => { setPreferences((current) => ({ ...current, filter: event.target.value as TodoFilter })); setVisibleTodoCount(TODO_PAGE_SIZE); }} className="h-10 bg-transparent text-sm outline-none">{filterOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><select aria-label="Todo 정렬" value={preferences.sort} onChange={(event) => setPreferences((current) => ({ ...current, sort: event.target.value as TodoSort }))} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none">{sortOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div>
        <NoteList notes={visibleTodos.slice(0, visibleTodoCount)} ownerName={employee?.name ?? "-"} onPatch={patchNote} onDelete={deleteNote} onShare={setShareTarget} emptyMessage="조건에 맞는 미완료 Todo가 없습니다."/>
        {visibleTodoCount < visibleTodos.length && <button type="button" onClick={() => setVisibleTodoCount((count) => count + TODO_PAGE_SIZE)} className="mt-3 w-full rounded-xl border border-slate-200 bg-white py-2 text-sm font-semibold text-blue-600">더 보기 ({visibleTodos.length - visibleTodoCount}건)</button>}
        <div className="mt-4 border-t border-slate-200 pt-3"><button type="button" onClick={() => setPreferences((current) => ({ ...current, completedOpen: !current.completedOpen }))} className="flex w-full items-center justify-between rounded-xl bg-white px-3 py-2 text-left"><span className="text-sm font-bold text-slate-700">완료한 일 <span className="text-slate-400">{completedTodos.length}</span></span>{preferences.completedOpen ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}</button>{preferences.completedOpen && <><NoteList notes={completedTodos.slice(0, visibleCompletedCount)} ownerName={employee?.name ?? "-"} onPatch={patchNote} onDelete={deleteNote} onShare={setShareTarget} emptyMessage="완료한 Todo가 없습니다." completed/>{visibleCompletedCount < completedTodos.length && <button type="button" onClick={() => setVisibleCompletedCount((count) => count + COMPLETED_PAGE_SIZE)} className="mt-3 w-full rounded-xl border border-slate-200 bg-white py-2 text-sm font-semibold text-blue-600">완료 내역 더 보기 ({completedTodos.length - visibleCompletedCount}건)</button>}</>}</div>
      </div>
      <div className="space-y-4"><ShareInvitationList refreshKey={sharingRefreshKey}/><ReferenceTaskSection/></div>
    </div>}

    {(preferences.tab === "schedule" || preferences.tab === "all") && <NoteSection title="📅 개인 일정" preset="memo" notes={schedules} ownerName={employee?.name ?? "-"} onPatch={patchNote} onDelete={deleteNote} onShare={setShareTarget}/>}
    {(preferences.tab === "memo" || preferences.tab === "all") && <NoteSection title="📝 메모" preset="memo" notes={memos} ownerName={employee?.name ?? "-"} onPatch={patchNote} onDelete={deleteNote} onShare={setShareTarget}/>}
...
function NoteList({ notes, ownerName, onPatch, onDelete, onShare, emptyMessage, completed = false }: { notes: PersonalNote[]; ownerName: string; onPatch: (note: PersonalNote, changes: Record<string, unknown>) => Promise<void>; onDelete: (note: PersonalNote) => Promise<void>; onShare: (note: PersonalNote) => void; emptyMessage: string; completed?: boolean }) {
...
function NoteSection({ title, preset, notes, ownerName, onPatch, onDelete, onShare }: { title: string; preset: NoteEditorPreset; notes: PersonalNote[]; ownerName: string; onPatch: (note: PersonalNote, changes: Record<string, unknown>) => Promise<void>; onDelete: (note: PersonalNote) => Promise<void>; onShare: (note: PersonalNote) => void }) {
  return <div className="mt-4 rounded-2xl bg-slate-50 p-4"><div className="flex items-center justify-between"><h3 className="font-bold text-slate-800">{title} <span className="text-slate-400">{notes.length}</span></h3><button type="button" aria-label={`${title} 추가`} onClick={() => openNoteEditor(preset)} className="flex h-8 w-8 items-center justify-center rounded-lg bg-white text-blue-600 shadow-sm"><Plus size={15}/></button></div><NoteList notes={notes} ownerName={ownerName} onPatch={onPatch} onDelete={onDelete} onShare={onShare} emptyMessage="등록된 항목이 없습니다."/></div>;
}
