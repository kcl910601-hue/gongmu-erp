...
export function ProjectMaterialAllocationsSection({ project }: { project: ProjectTarget }) {
  ...
  const load = useCallback(async () => {
    ...
    const allocationResult = await allocationResponse.json() as { allocations?: ProjectAllocation[]; summary?: ProjectMaterialCostSummary; canManage?: boolean; error?: string };
    ...
    setAllocations(allocationResult.allocations ?? []); setSummary(allocationResult.summary ?? emptySummary); setContracts(contractResult.contracts ?? []); setCanManage(allocationResult.canManage === true);
  }, [project.id]);
