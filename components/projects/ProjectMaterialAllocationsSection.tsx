...
export function ProjectMaterialAllocationsSection({ project }: { project: ProjectTarget }) {
  ...
  const load = useCallback(async () => {
    ...
    const allocationResult = await allocationResponse.json() as { allocations?: ProjectAllocation[]; summary?: ProjectMaterialCostSummary; unallocatedTons?: number; canManage?: boolean; error?: string };
    ...
    setAllocations(allocationResult.allocations ?? []); setSummary(allocationResult.summary ?? emptySummary); setUnallocatedTons(Number(allocationResult.unallocatedTons ?? 0)); setContracts(contractResult.contracts ?? []); setCanManage(allocationResult.canManage === true);
  }, [project.id]);
